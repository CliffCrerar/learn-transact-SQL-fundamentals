---
name: Home
route: /
---

import SubTitle from '../components/frontmatter/subtitle.jsx'
import Image from '../components/img'
import SQLEditor from '../components/embeded-sql-editor'
import './index.scss';

# Learn Transact SQL Fundamentals

<style jsx="true">{`
		.img-div {
			display: flex;
			justify-content: space-around;
			align-items: center;
		}

		[alt="Learn-SQL"] {
			position: relative;
			width: 40%;
		}

		.myButton {
			-moz-box-shadow: inset 0px 34px 0px -15px #b54b3a;
			-webkit-box-shadow: inset 0px 34px 0px -15px #b54b3a;
			box-shadow: inset 0px 34px 0px -15px #b54b3a;
			background-color: #a73f2d;
			cursor: pointer;
			color: #ffffff;
			font-size: 15px;
			padding: 9px 23px;
			text-decoration: none;
			text-shadow: 0px -1px 0px #7a2a1d;
			border: 0px Solid transparent;
			border-radius: 5px;
		}
		.myButton:hover {
			background-color: #b34332;
		}
		.myButton:active {
			position: relative;
			top: 1px;
		}

	`}</style>

<SubTitle/>


<Image 
InjectElement={()=>(
	<a href="/introduction">
    	<button className="myButton">GET STARTED</button>
	</a>
	)
}
src={require('../assets/learnsql.png')} 
alt={'Learn-SQL'} 
divClass={{display: 'flex', justifyContent:'space-around', flexFlow:'row-reverse', alignItems: 'center'}}
/>

## Exiting new features

We are in the process of implementing exiting new features for documentation to improve user experience.

There will be a lot more content available and we plan in implementing a live SQL editor that go along with the lessons. This means no more tedious time spent on installing SQL server.



### Planned Content:

#### List topics that are coming soon:

- Merge
- Unions
- Cursors
- Triggers
- Declaring and using variables
- Stored procedures
- Common table expressions
- Conditional operations (IF, CASE)
- Loops
- Data control language
- Joining tables
- Database Views
- Data modeling (elaboration)
- Systems development life cycle
- Sub queries